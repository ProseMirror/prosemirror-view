...
export class EditorView {
  ...
  private updateStateInner(state: EditorState, prevProps: DirectEditorProps) {
    ...
    if (updateSel) {
      ...
      if (
        forceSelUpdate ||
        !(this.input.mouseDown && this.domObserver.currentSelection.eq(this.domSelectionRange()) && anchorInRightPlace(this))
      ) {
    ...
    if ((this.dragging as Dragging)?.node && !prev.doc.eq(state.doc)) this.updateDraggedNode(this.dragging as Dragging, prev);
...
function updateCursorWrapper(view: EditorView) {
  if (view.markCursor) {
    ...
    view.cursorWrapper = {
      dom,
      deco: Decoration.widget(view.state.selection.from, dom, { raw: true, marks: view.markCursor } as any),
    };
  } else {
